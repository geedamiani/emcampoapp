...
export default async function PublicDashboardPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [
    ...
    supabase.from('match_events').select('*, players(name)').eq('user_id', userId),
    supabase.from('match_players').select('player_id, match_id, starter').eq('user_id', userId),
    ...
    <div className="mx-auto max-w-lg px-4 py-5">
      <div className="mb-5">
        ...
        <p className="text-sm text-muted-foreground">
          {team.name} &middot; {totalMatches} {totalMatches === 1 ? 'partida' : 'partidas'} &middot; {wins}V {draws}E {losses}D
        </p>
      </div>
